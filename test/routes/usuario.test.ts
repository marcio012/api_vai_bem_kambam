...
describe('userRoute', () => {
  it('deve responder com HTTP status 404 quando não há usuário', async () => {
    const result = await request(app).get(`/users/${user.nomeUsuario}`)
  ...
  it('deve atualizar o usuário Marcio', async () => {
    ...
    const result = await request(app).patch(`/users/marcio`).send(user)
  ...
  it('deve retornar 404 se o usuário não existir', async () => {
    ...
    const result = await request(app).patch(`/users/bill`).send(user)
    expect(result.status).toEqual(404)
  })
  it('deve remover um usuário existente', async () => {
    const result = await request(app).del(`/users/${user.nomeUsuario}`)

    expect(result.status).toEqual(204)
  })
  it('deve retornar 404 quando estiver tentando remover um usuário que não existe', async () => {
    const result = await request(app).del(`/users/Mary`)
